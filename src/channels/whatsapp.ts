...
export class WhatsAppChannel implements Channel {
      ...
      for (const msg of messages) {
        ...
        if (groups[chatJid]) {
          ...
          if (hasMedia && !isBotMessage) {
            ...
            const downloaded = await this.downloadWAMedia(
              msg,
              groupFolder,
              imageMsg,
              videoMsg,
          ...
          const finalContent =
            ...
            (imageMsg
              ...
              : videoMsg
                ? '[Video]'
                : documentMsg
                  ? `[Document: ${documentMsg.fileName || 'file'}]`
                  : '');
  ...
  private async downloadWAMedia(
    ...
    groupFolder: string,
    imageMsg:
      | { mimetype?: string | null; caption?: string | null }
      | null
      | undefined,
    videoMsg:
      | { mimetype?: string | null; caption?: string | null }
    ...
    try {
      ...
      logger.info(
        { filePath, size: buffer.length, mediaType },
        'WhatsApp media saved',
      );
